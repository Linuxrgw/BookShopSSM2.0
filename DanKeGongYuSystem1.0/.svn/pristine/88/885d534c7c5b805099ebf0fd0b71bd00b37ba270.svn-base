	...
	function gotoPage() {
		...
		if (!isNaN(pageNum)) {
			...
			if (pageNum>0 && pageNum <= totalPage) {
                window.location.href='BackAllOwnerSvl?current='+pageNum;
			} else {
					...
					<div id="box_border">
						...
						<div id="box_center">
							用户名&nbsp;&nbsp;<input type="text" id="fyZldz" name="fangyuanEntity.fyZldz" class="ui_input_txt02" />						
						         区&nbsp;&nbsp;<input type="text" id="fyZldz" name="fangyuanEntity.fyZldz" class="ui_input_txt02" />						
							性别
			...
			<div class="ui_content">
				<div class="ui_tb">
					<table class="table" cellspacing="0" cellpadding="0" width="100%" align="center" border="0">
						...
						<c:forEach var="owner" items="${requestScope.page.list}">
							<tr>
								<td><input type="checkbox" name="IDCheck" value="14458579642011" class="acb" /></td>
								<td>${owner.ownerid }</td>
								<td>${owner.ownername }</td>
								<td>${owner.ownerpwd }</td>								
								<td>${owner.ownersex }</td>
					...
					<div class="ui_frt">
						<!--    如果是第一页，则只显示下一页、尾页 -->
							...
							<input type="button" value="首页" class="ui_input_btn01" 
							    onclick="window.location.href='BackAllOwnerSvl?current=1'" />
							<input type="button" value="上一页" class="ui_input_btn01" 
							    onclick="window.location.href='BackAllOwnerSvl?current=${requestScope.page.currentPage-1}'" />
							<input type="button" value="下一页" class="ui_input_btn01"
								onclick="window.location.href='BackAllOwnerSvl?current=${requestScope.page.currentPage+1}'" />
							<input type="button" value="尾页" class="ui_input_btn01"
								onclick="window.location.href='BackAllOwnerSvl?current=${requestScope.page.totalPage}'" />					
