	...
	$(document).ready(function() {
		...
		$("#submitbutton").click(function() {
			if(validateForm()){
				...
				window.location.href="AddRoomSvl";
			}
		});
...
// 		}
		if($("#address").val()==""){
// 			alert("友情提示:填写房源位置");
			art.dialog({icon:'error', lock:true, title:'友情提示', drag:false, resize:false, content:'填写房源位置', ok:true,});
			return false;
		}
		else if($("#comment").val()==""){
// 			alert("友情提示:填写房源描述");
			art.dialog({icon:'error', lock:true,title:'友情提示', drag:false, resize:false, content:'填写房源描述', ok:true,});
			return false;
		}
		else if($("#area").val()==""){
// 			alert("友情提示:填写房源面积");
			art.dialog({icon:'error', lock:true,title:'友情提示', drag:false, resize:false, content:'填写房源面积', ok:true,});
			return false;
		}
		else if($("#jzarea").val()==""){
// 			alert("友情提示:填写房源计租面积");
			art.dialog({icon:'error',lock:true,time:'3000',lock:true, title:'友情提示', drag:false, resize:false, content:'填写计租面积', ok:true,});
			return false;
		}
		else if($("#type").val()==""){
// 			alert("友情提示:填写房源户型");
			art.dialog({icon:'error',lock:true, title:'友情提示', drag:false, resize:false, content:'填写户型', ok:true,});
			return false;
		}
		else if($("#subway").val()==""){
// 			alert("友情提示:填写房源位置");
			art.dialog({icon:'error', lock:true,title:'友情提示', drag:false, resize:false, content:'填写地铁', ok:true,});
			return false;
		}
		else if($("#rentdate").val()==""){
// 			alert("友情提示:填写租赁日期");
			art.dialog({icon:'error', lock:true,title:'友情提示', drag:false, resize:false, content:'填写可租赁日期', ok:true,});
			return false;
		}
		else if($("#sta").val()==""){
// 			alert("友情提示:填写状态");
			art.dialog({icon:'error',lock:true, title:'友情提示', drag:false, resize:false, content:'填写状态', ok:true,});
			return false;
		}
		else if($("#price").val()==""){
// 			alert("友情提示:填写价格");
			art.dialog({icon:'error',lock:true, title:'友情提示', drag:false, resize:false, content:'填写价格', ok:true,});
			return false;
		}
		else{return true;}
	}
